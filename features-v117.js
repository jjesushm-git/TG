...
// Mensajería
let activeRecipient="",messageChannel=null,messagePoll=null,messageBusy=false,messageSignature="",messageCache=[],editingMessageId="",profileEmails=new Map();
...
async function loadMessages(force=false){
  ...
  let query=sb.from("messages").select("*").order("created_at",{ascending:true}).limit(1000);
  if(isRecess())query=query.eq("room","recreo");else{const filter=`and(sender_id.eq.${me},recipient_id.eq.${activeRecipient}),and(sender_id.eq.${activeRecipient},recipient_id.eq.${me})`;query=query.eq("room","direct").or(filter)}
  const {data,error}=await query;messageBusy=false;if(error){toast(error.message);return false}
  ...
  messageCache=data;const box=$("#messageList");box.innerHTML=data.map(m=>{const file=isTransferNow(m.body),mine=m.sender_id===me,content=file?`<a class="transfer-message" href="${escapeHtml(m.body)}" target="_blank" rel="noopener noreferrer">📎 Archivo en TransferNow<br><small>Disponible temporalmente · Abrir o descargar</small></a>`:`<span>${escapeHtml(m.body)}</span>`,edited=m.updated_at&&new Date(m.updated_at)>new Date(m.created_at),author=isRecess()?`<b class="message-author">${mine?"Tú":escapeHtml(profileEmails.get(m.sender_id)||"Usuario")}</b>`:"";return `<div class="message-line ${mine?"mine-line":""}">${mine?`<button class="message-tool pencil-tool" data-edit-message="${m.id}" aria-label="Editar mensaje" title="Editar mensaje">✏️</button>`:""}<article class="message ${mine?"mine":""}" data-message-card="${m.id}">${author}${content}<time>${stamp(m.created_at)}${edited?" · editado":""}</time></article>${mine?`<button class="message-tool trash-tool" data-delete-message="${m.id}" aria-label="Borrar mensaje" title="Borrar mensaje">🗑️</button>`:""}</div>`}).join("")||"<p class='muted'>Aún no hay mensajes.</p>";
...
$("#messageRecipient").onchange=async e=>{activeRecipient=e.target.value;localStorage.setItem(lastChatKey(),activeRecipient);messageSignature="";await loadMessages(true)};
$("#messageForm").onsubmit=async e=>{e.preventDefault();const body=$("#messageBody").value.trim();if(!activeRecipient)return toast("Selecciona un usuario");const row=isRecess()?{sender_id:state.user.id,recipient_id:null,room:"recreo",body}:{sender_id:state.user.id,recipient_id:activeRecipient,room:"direct",body};const {error}=await sb.from("messages").insert(row);if(error)return toast(error.message);$("#messageBody").value="";await loadMessages()};
...
$("#clearChatBtn").onclick=async()=>{if(!activeRecipient)return;const group=isRecess(),question=group?"En Recreo se borrarán solamente todos tus mensajes. ¿Continuar?":"Se borrará toda esta conversación para las dos personas. ¿Continuar?";if(!confirm(question))return;const {error}=await sb.rpc("clear_chat",{target_user:group?null:activeRecipient,target_room:group?"recreo":"direct"});if(error)return toast(error.message);messageSignature="";await loadMessages(true);toast(group?"Tus mensajes de Recreo fueron borrados":"Conversación borrada")};
async function paperBasketAnimation(card,trash){
  if(!card||!trash||matchMedia("(prefers-reduced-motion: reduce)").matches)return;
  const stage=document.createElement("div");stage.className="delete-stage";stage.innerHTML='<div class="tear-note"></div><div class="basket-bin">🗑️<small>BOTE</small></div>';
